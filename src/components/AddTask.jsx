...
console.log("hi")
  const handleSave = async(data) => {
    ...
    await axisoInstance.post("/api/user/add-task", {taskName:taskName, description:description, date:date}, config)
    .then((res)=>{
        if (res.data.success) {
  ...
  return (
    <div className="flex justify-center h-screen items-center flex-col my-auto">
      ...
      <form onSubmit={handleSubmit(handleSave)} className="w-[500px] mt-10">
        <div>
          <input
            type="text"
        ...
        <Controller
            ...
            render={({ field: { onChange, value } }) => (
                <DatePicker
                  ...
                  className="p-2 px-5 my-2 h-16 w-[500px] rounded-lg bg-[#efefef]" 
                />
              )}
          />
          {errors.datePicker && (
